...
contract QQuestP2PCircleMembership is ERC1155, EIP712, AccessControl {
    ...
    constructor(string memory name, string memory version, string memory uri, address _trustedEntity)
        ERC1155(uri)
        EIP712(name, version)
    {
    ...
    function burnNFT(address user, uint256 _tokenId, uint8 count) public onlyRole(DEFAULT_ADMIN_ROLE) {
